...
if (isset($_POST['rabies_tag'])) {
  $colname_rs_animals = (get_magic_quotes_gpc()) ? $_POST['rabies_tag'] : addslashes($_POST['rabies_tag']);
}
mysql_select_db($database_JLS_conn, $JLS_conn);
$query_rs_animals = sprintf("SELECT * FROM animals WHERE rabies_type_test = %s", GetSQLValueString($colname_rs_animals, "text"));
...
<div id="holder">
  ...
  <div id="content_nav"><!-- InstanceBeginEditable name="content" -->
    ...
    <table width="520" border="5">
      ...
      <?php do { ?>
        <tr>
          <td><?php echo $row_rs_animals['rabies_type_test']; ?></td>
          <td><a href="animal_change.php?animal_id=<?php echo $row_rs_animals['animal_id']; ?>"><?php echo $row_rs_animals['name']; ?></a></td>
          <td><?php echo $row_rs_animals['animal_type']; ?></td>
          <td><?php echo $row_rs_animals['status']; ?></td>
        </tr>
        <?php } while ($row_rs_animals = mysql_fetch_assoc($rs_animals)); ?>
    </table>
    <p align="left">&nbsp;</p>
    <p align="left">&nbsp;</p>
