...
function PostResult($thePage, $theErrors, $valPage) {
  ...
  if (strpos($thePostURL,"://") === false)  {
    ...
    $thePostURL = $schema."://".str_replace("%2F","/",rawurlencode($host.$thePostURL)).$urlParams;
  }
  ...
  if (!(!$required && $value==""))  {
    ...
    if ($doDate)  {
      ...
      if ($isValid)  {
        ...
        if ($dateVar == -1 || is_numeric($value) || (strpos($value, "/") == strrpos($value, "/") && strpos($value, "/") != false))
          $isValid = false;
        if ($dateMin != "")  {
...
function WAValidateRT($value,$notAllowed,$required,$number)  {
  ...
  $tempVal = $augValue;
  if (!(!$required && $value==""))  {
    $notAllowed = split(", ", $notAllowed);
    foreach ($notAllowed AS $x)  {
      if ($x != "") {
        $notAllowedInfo = split("\|", $x);
