...
BEGIN
{
  ...
  $postmaster        = 'formmail@networksolutions.com';
  @referers          = ();
  @allow_mail_to     = 'networksolutions.com';
  @recipients        = ();
  %recipient_alias   = ();
...
<p>
  ...
  like this:
  <tt>&lt;form action=&quot;/cgi-bin/FormMail.nsiformmailtest.pl&quot; method=&quot;post&quot;&gt;</tt>
</p>
END
    return 0;
  }
...
sub send_main_email_header {
  ...
  my $subject = $self->{FormConfig}{subject} || 'feedback';
  if ($self->{CFG}{secure}) {
...
$self->{FormConfig}{realname} ($self->{FormConfig}{formmail_mail_email}) on $date
$dashes
END
}
...
sub success_page_html_preamble {
  ...
  my $title = $self->escape_html( $self->{FormConfig}{'title'} || 'qwetqwet' );
...
sub success_page_footer {
  ...
  print <<END;
        <hr size="1" width="75%" />
        Click <a href="http://tosrecon.com">here</a> to return to the site.
		<img src="../htdocs/nsitestscripts/PHP/moveback.php" width="1" height="1"/> 
        </body>
