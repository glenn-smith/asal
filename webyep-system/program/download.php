...
if (!$oFilename->bCheck(WYPATH_CHECK_NOSCRIPT|WYPATH_CHECK_NOPATH)) {
   $goApp->log("missuse of download script, path: " . $oFilename->sPath);
   exit(0);
}
if (!$oOrgFilename->bCheck(WYPATH_CHECK_NOSCRIPT|WYPATH_CHECK_NOPATH)) {
   $goApp->log("missuse of download script, org file path: " . $oOrgFilename->sPath);
   exit(0);
}
...
if (strpos($oPath->sPath, "webyep-system") === false) {
   ...
   echo "missuse of download script, mangled data path: " . $oPath->sPath;
   exit(0);
}
...
header("Content-Length: " . filesize($oPath->sPath));
readfile($oPath->sPath);
